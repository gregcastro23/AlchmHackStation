...
| **Phase 2** | **Live Matchmaking & Arena Real-Time** | Pentacles, HackStation, ASOL | **COMPLETED / PRs OPEN** | HackStation [#7](https://github.com/gregcastro23/AlchmHackStation/pull/7)<br>ASOL [#37](https://github.com/gregcastro23/alchm-agents-solana/pull/37)<br>Pentacles: `main` | SpacetimeDB subscription isolation; `JingArenaDeck` simulator with `[SIMULATED ARENA SANDBOX]` tag; server enum parity (`Open`, `Resolved`, `Cancelled`); sub-400ms build; 0 lint errors on touched files (all 12 remediated in `aba1026`); ASOL `/api/vessel/summary` implemented with 15/15 Vitest tests passing. Ready to merge for Phase 3 entry. |
...
| **Phase 4** | **Telemetry, Astrometry & Full Release** | All 4 Repos | **QUEUED** | Cross-ecosystem tag `v1.0-jing`<br>WTEN [#849](https://github.com/gregcastro23/WhatToEatNext/pull/849) (On Hold) | Astrometry reconciliation drift engine (<1500ms); CU profiling; clean desktop build; merge WTEN Phase 0 spec alignment. |
